...
using JSON
using StaticArrays
export visualize_unitcell_atoms,visualize_unitcell_atoms0,matrix_to_latex,read_json,read_config,visualize_beadcell,calculate_gr!,DataProcessor,RadialDistribution,Normalize_gr!,SpatialDistribution,calculate_pr!,Normalize_pr!,Trajectory,calculate_traj!,fix_traj!,clear_traj!,write_config_file
...
function write_config_file(basepath::String, config_dict::Dict)
    open(joinpath(basepath, "Config.txt"), "w") do logfile
        # 首先处理顶层非Dict的键值对（排除已知需要特殊处理的键）
        for (key, value) in config_dict
            if !(value isa Dict)
                write(logfile, "$key=$value\n")
            end
        end
        # 递归处理嵌套字典
        function write_nested_dict(dict::Dict, level::Int=0)
            for (key, value) in dict
                if value isa Dict
                    # 字典类型：添加标题并递归处理
                    write(logfile, "\n$key:\n")
                    write_nested_dict(value, level + 1)
                else
                    # 非字典类型：处理特殊逻辑
                    if key == "avgdistance" && haskey(dict, "nparticle") && dict["nparticle"] == 1
                        write(logfile, "avgdistance=None\n")
                    elseif key == "simulation_time"
                        write(logfile, "simulation time:$(value)s\n")
                    else
                        write(logfile, "$key=$value\n")
                    end
                end
        ...
        for (key, value) in config_dict
            if value isa Dict
                write(logfile, "\n$key:\n")
                write_nested_dict(value)
            end
        end
        # 最后处理Comments（如果有）
        if haskey(config_dict, "Comments")
            write(logfile, "\nComments\n$(config_dict["Comments"])\n")
        end
    end
end
